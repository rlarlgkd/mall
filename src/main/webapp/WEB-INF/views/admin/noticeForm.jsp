...
<head>
	...
	<script>
		function form_check() {
			if ($('#notitle').val().length == 0) {
				alert("제목 입력은 필수사항입니다.");
				$('#notitle').focus();
				return;
			}

			if ($('#nocontent').val().length == 0) {
		...
		<div id="cartView">
			<table class="notice" width="800">
				<form id="frm" action="noticeWrite" method="post">
					<tr>
						<td>제목<br />
							<input type="text" name="notitle" id="notitle" size="50">
						</td>
					</tr>
					<tr>
						<td>내용<br />
							<textarea class="form-control" name="nocontent" id="nocontent" rows="10"></textarea>
						</td>
						<script>
